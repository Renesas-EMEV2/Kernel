...
static DEFINE_SPINLOCK(usb_stat_lock);
static int usb_stat_save = EMXX_USB_OFF;
...
int emxx_battery_usb_state(int state)
{
	...
	spin_lock_irqsave(&usb_stat_lock, flags);
	...
	} else if (battery_data->usb_state != state) {
		switch (state) {
		case EMXX_USB_DEVICE:
#ifdef CONFIG_EMXX_ANDROID
...
err:
	spin_unlock_irqrestore(&usb_stat_lock, flags);
	return ret;
}
...
static int emxx_battery_probe(struct platform_device *pdev)
{
	...
	emxx_battery_usb_state(usb_stat_save);
