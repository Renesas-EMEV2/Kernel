...
struct emxx_cam_frames {
	struct kfifo_lock *enq;
	struct kfifo_lock *queingq;
	wait_queue_head_t cam_queing_waitq;
...
static inline int camif_chg_pinsel(int on)
{
	unsigned long val;

	FNC_ENTRY;

	val = readl(CHG_PULL18) & ~0xffff0000;

	if (on) {
		/*input enable, PU/PD setting*/
		/*all input enable and PU/PD disable, except for CLKO*/
		writel(val | 0x44400000, CHG_PULL18);
		writel(0x44444444, CHG_PULL19);
	} else {
		writel(val | 0x33300000 , CHG_PULL18);
		writel(0x33333333, CHG_PULL19);
	}
...
static inline int camif_hw_start(void)
{
	FNC_ENTRY;
	...
	emxx_clkctrl_on(EMXX_CLKCTRL_CAMPCLK);

	FNC_EXIT_N;
	return 0;
}

static inline int camif_hw_stop(void)
{
	FNC_ENTRY;

	emxx_clkctrl_off(EMXX_CLKCTRL_CAMPCLK);
	emxx_reset_device(EMXX_RST_CAM);
	emxx_reset_device(EMXX_RST_CAM_SAFE);
...
static inline int camif_set_frame(__u32 index, __u32 frame)
{
	int ret = 0;
	struct camif_reg ca;
	FNC_ENTRY;
	...
	if (frame == 0) {
		...
		outl(ca.yplane_a, CA_YPLANE_B);
		outl(ca.uvplane_a, CA_UVPLANE_B);
		outl(ca.vplane_a, CA_VPLANE_B);
	}
...
static const struct emxx_cam_fmt emxx_cam_formats[] = {
	...
	/*need to reconfirm whether support this format*/
...
static inline unsigned int kfifo_try_get(struct kfifo_lock *fifo_l,
					 unsigned char *buffer,
					 unsigned int len)
{
	unsigned long flags;
	unsigned int i = len;
	unsigned int ret;
	struct kfifo *fifo = &fifo_l->kfifo;
	FNC_ENTRY;

	spin_lock_irqsave(&fifo_l->lock, flags);

	ret = __kfifo_try_get(fifo, buffer, len);
	len = fifo->in - fifo->out;

	spin_unlock_irqrestore(&fifo_l->lock, flags);
...
static inline void deq_done_pushq(unsigned int q, int done)
{
	unsigned int ret;
	FNC_ENTRY;
...
static inline void queing_pushq(unsigned int q)
{
	unsigned int ret;
	FNC_ENTRY;
...
static inline unsigned int queing_pullq(void)
{
	unsigned int buf;
	unsigned int ret;
	FNC_ENTRY;
...
static inline int cam_waiton(struct emxx_cam *cam, int *index,
			     int non_blocking, int timeout_msec)
{
	int ret = 0;
	__u32 state;
	unsigned int peep_index;
	int timeout;
	FNC_ENTRY;

	if (timeout_msec > 0)
		timeout = msecs_to_jiffies(timeout_msec);

	do {
		if (non_blocking) {
			if (!deq_done_peepq(&peep_index)) {
				ret = -EAGAIN;
				break;
			}
		...
		state = cam->grab->buff[peep_index].state;
		if (state == CAM_BUF_DONE) {
			*index = peep_index;
			break;
		} else if (CAM_BUF_BREAK == state) {
			err("%s: buffer break.\n", CAM_NAME);
			ret = -EIO;
			break;
		}
...
static int kfifo_lock_alloc(struct kfifo_lock **kfifo_l)
{
	struct kfifo_lock *kfifo_l_tmp;
	int ret;

	kfifo_l_tmp = kzalloc(sizeof(*kfifo_l_tmp), GFP_KERNEL);
	if (NULL == kfifo_l_tmp)
		return -ENOMEM;

	ret = kfifo_alloc(&kfifo_l_tmp->kfifo,
		sizeof(int) * EMXX_CAM_MAX_BUFNBRS, GFP_KERNEL);
	if (ret) {
		kfree(kfifo_l_tmp);
		kfifo_l_tmp = NULL;
		return -ENOMEM;
	...
	spin_lock_init(&kfifo_l_tmp->lock);
...
static void kfifo_lock_free(struct kfifo_lock *kfifo_l)
{
	if (kfifo_l == NULL)
		return;

	kfifo_free(&kfifo_l->kfifo);
