...
#define CAM_NAME "emxx_rear_camera"
#if 1
...
#define MODULE_NAME "emxx_rear_camera"
				  ...
				  struct v4l2_buffer *b)
{
	...
	__kfifo_get(filled_fifo, (unsigned char*)&buffer_index,sizeof(int));
	memset(b, 0, sizeof(*b));
...
static int emxx_cam_probe(struct platform_device *devptr)
{
	...
	/*register cam hw*/
	ret = emxx_rear_cam_hw_register(&sensor_ops);
	if (ret)
	{
...
MODULE_PARM_DESC(video_nr, "video device to register (0=/dev/video2, etc)");
