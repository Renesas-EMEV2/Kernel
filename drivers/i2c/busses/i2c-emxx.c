...
static spinlock_t i2c_lock[I2C_NR];
static spinlock_t i2c_irq_lock[I2C_NR];
...
static i2c_ctrl_t emxx_i2c_ctrl0 = {
	.smc = CONFIG_I2C_EMXX_SMC0,	/* mode */
	.dfc = CONFIG_I2C_EMXX_DFC,	/* digital filter */
};

#ifdef CONFIG_I2C_EMXX_ENABLE_CH2
static i2c_ctrl_t emxx_i2c_ctrl1 = {
	.smc = CONFIG_I2C_EMXX_SMC1,	/* mode */
	.dfc = CONFIG_I2C_EMXX_DFC,	/* digital filter */
};
#endif
...
static int emxx_i2c_wait_for_pin(struct i2c_adapter *adap, u16 *status)
{
	...
	do {
		...
		if (irq) {
			...
			spin_lock_irq(&i2c_irq_lock[i2c_dev->ch]);
			pending = i2c_pending[i2c_dev->ch];
			i2c_pending[i2c_dev->ch] = 0;
			spin_unlock_irq(&i2c_irq_lock[i2c_dev->ch]);
	...
	struct pt_regs *regs)
{
	...
	spin_lock(&i2c_irq_lock[i2c_dev->ch]);
	i2c_pending[i2c_dev->ch] = 1;
	spin_unlock(&i2c_irq_lock[i2c_dev->ch]);
...
static int emxx_i2c_probe(struct platform_device *dev)
{
	...
	spin_lock_init(&i2c_lock[dev->id]);
	spin_lock_init(&i2c_irq_lock[dev->id]);
