...
static unsigned char hw_version = 0;
/*
...
/*******************************************************	
功能：	Function as i2c_master_receive
	读取从机数据
...
static int goodix_ts_power(struct goodix_ts_data * ts, int on)
{
	...
	switch(on) 
	{
		case 0:
			...
			msleep(5);
			if(gpio_get_value(SHUTDOWN_PORT))	//has been suspended
				ret = 0;
			break;
		case 1:
			gpio_set_value(SHUTDOWN_PORT, 0);
			msleep(5);
			if(gpio_get_value(SHUTDOWN_PORT))	//has been waked up
				ret = -1;
			else 
				msleep(200);
			break;	
...
/*
	...
	hw_version = get_emxx_hw_version();
*/
	if (board_version == 0 || (board_version == 1 && hw_version == 0)) {
		...
		if (!ret){
			printk("goodix_ts_driver is ok\n");
		}else{
			printk("goodix_ts_driver failed");
		}
	}
