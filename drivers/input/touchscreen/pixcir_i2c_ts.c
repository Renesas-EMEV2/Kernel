 ...
 *
 * pixcir_i2c_ts.c V2.3.2        fix m48 show rawdata bug
 *
...
static struct i2c_dev *i2c_dev_get_by_minor(unsigned index)
{
	...
	spin_lock(&i2c_dev_list_lock);
	printk(KERN_ERR "i2c-dev: get_by_minor %u\n", index);
...
#endif
	posx1 = 1024-(int)((long)posx1 * 800 / 920);
	posy1 = (int)((long)posy1 * 480 / 360);
	posx2 = 1024-(int)((long)posx2 * 800 / 920);
	posy2 = (int)((long)posy2 * 480 / 360);
...
static ssize_t pixcir_proc_read(struct file *filp,
		char *buffer, size_t length, loff_t *offset)
{
	return 0;
}
static ssize_t pixcir_proc_write(struct file *filp,
...
static struct proc_dir_entry *pixcir_proc_file;
static struct file_operations pixcir_proc_ops = {
	.read = pixcir_proc_read,
	.write = pixcir_proc_write,
};

static void create_pixcir_proc_files(void)
{
	pixcir_proc_file = create_proc_entry("driver/pixcir", 0644, NULL);
	if (pixcir_proc_file) {
		//pixcir_proc_file->owner = THIS_MODULE;
		pixcir_proc_file->proc_fops = &pixcir_proc_ops;
	} else
		printk(KERN_INFO "proc file create failed!\n");
}
static void remove_pixcir_proc_file(void)
{
	remove_proc_entry("driver/pixcir", NULL);
}
...
static int pixcir_open(struct inode *inode, struct file *file)
{
	...
	if(i2cdev_check_addr(client->adapter,SLAVE_ADDR)) {
		printk(" i2cdev_check_addr error");
	 	return -EBUSY;
	}
...
static ssize_t pixcir_write(struct file *file, const char __user *buf,size_t count, loff_t *ppos)
{
	...
	switch(status_reg)
	{
		...
		if(ret){
			ret = -1;
			printk("READ_EEPROM,copy_from_user error, ret = %d\n",ret);
			break;
		}
		wr2eep[0] = SPECOP;
		wr2eep[1] = 1;
		...
		if(ret != 4){
			ret = -1;
			printk("READ_EEPROM wr2eep,i2c_master_send, ret = %d\n",ret);
			break;	
		}
		}
		break;
...
{ 	.driver =
		{
			.owner = THIS_MODULE,
			.name = "pixcir_i2c_ts_driver_v2.3.2",
		}, 
...
static int __init pixcir_i2c_ts_init(void)
{
	int ret = 0;
        ...
        board_version = 4;/* get_emxx_board_version(); */
