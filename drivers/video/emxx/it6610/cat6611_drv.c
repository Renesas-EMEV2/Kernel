...
// Video Function
///////////////////////////////////////////////////////////
void SetInputMode(BYTE InputColorMode, BYTE InputSignalType);
void SetCSCScale(BYTE bInputMode,BYTE bOutputMode);
void SetupAFE(BOOL HighFreqMode);
void FireAFE();
///////////////////////////////////////////////////////////
// Audio Function
///////////////////////////////////////////////////////////
SYS_STATUS SetAudioFormat(BYTE NumChannel, BYTE AudioEnable, ULONG SampleFreq, BYTE AudSWL, BYTE AudioCatCode);
SYS_STATUS SetNCTS(ULONG PCLK, ULONG Fs);
void SetAudioChannel() ;
void AdjustAudioSampleClock() ;

///////////////////////////////////////////////////////////
// DDC
///////////////////////////////////////////////////////////
void ClearDDCFIFO();
void GenerateDDCSCLK() ;
void AbortDDC();
void AdjustDDC(BOOL bEnable) ;
...
// InfoFrame
///////////////////////////////////////////////////////////
SYS_STATUS SetAVIInfoFrame(AVI_InfoFrame *pAVIInfoFrame);
SYS_STATUS SetAudioInfoFrame(Audio_InfoFrame *pAudioInfoFrame);
///////////////////////////////////////////////////////////
// HDCP Authentication
///////////////////////////////////////////////////////////
SYS_STATUS HDCP_EnableEncryption();
void HDCP_Auth_Fire();
void HDCP_StartAnCipher();
void HDCP_GenerateAn();
SYS_STATUS HDCP_GetBCaps(PBYTE pBCaps ,PUSHORT pBStatus);
SYS_STATUS HDCP_GetBKSV(BYTE *pBKSV);
SYS_STATUS HDCP_Authenticate();
void HDCP_ResumeAuthentication();
void HDCP_ReadRi() ;

void HDCP_ResetAuthenticate() ;
void HDCP_StartAnCipher() ;
void HDCP_StopAnCipher() ;
SYS_STATUS HDCP_GetVr(BYTE *pVr) ;
SYS_STATUS HDCP_Authenticate_Repeater() ;
SYS_STATUS HDCP_VerifyIntegration() ;
SYS_STATUS HDCP_GetKSVList(BYTE *pKSVList,BYTE cDownStream) ;
SYS_STATUS HDCP_CheckSHA(BYTE M0[],USHORT BStatus,BYTE KSVList[],int devno,BYTE Vr[]) ;
void HDCP_Reset() ;
...
EnableAudioOutput(unsigned long VideoPixelClock, unsigned long AudioSampleClock, int AudioChannel, BOOL bSPDIF)
{
    ...
    SetAudioFormat(AudioChannel,bAudioChannelEnable,AudioSampleClock,AUDIO_DATA_LEN,0) ;
...
SetAudioFormat(BYTE NumChannel, BYTE AudioEnable, ULONG SampleFreq, BYTE AudSWL, BYTE AudioCatCode)
{
    ...
    if( HDMITX_ReadI2C_Byte(REG_INT_CTRL)&B_IDENT_6612 )
    {
        ...
        {
        #if defined (USE_MCLK_128FS)
            #pragma message("external MCLK sampling 128Fs")
            fs |= B_EXT_128FS|B_EXT_MCLK ;
        #elif defined ( USE_MCLK_512FS)
            #pragma message("external MCLK sampling 512Fs")
            fs |= B_EXT_512FS|B_EXT_MCLK ;
        #elif defined ( USE_MCLK_1024FS)
            #pragma message("external MCLK sampling 1024Fs")
            fs |= B_EXT_1024FS|B_EXT_MCLK ;
        #else
            #pragma message("external MCLK sampling 256Fs")
...
SetNCTS(ULONG PCLK, ULONG Fs)
{
    ...
    ErrorF("SetNCTS(%ld,%ld): Fs*128 = %ld\n",PCLK,Fs,Fs*128) ;
    ...
    if(TimeOut == 0 )
    {
    	...
    	if(bHDMIMode)
    	{
    		if((BStatus & B_CAP_HDMI_MODE)==0)
    		{
    			...
    			// return ER_FAIL ;
    			// ignore the HDCP BStatus not match, continue.
    		}
    	}
    	else
    	{
...
void SHA_Simple(void *p,LONG len,BYTE *output)
{
    ...
    for( i = 0 ; i < 20 ; i+=4 )
    {
        output[i  ] = (BYTE)(digest[i/4]      &0xFF) ;
        output[i+1] = (BYTE)((digest[i/4]>>8) &0xFF) ;
        output[i+2] = (BYTE)((digest[i/4]>>16)&0xFF) ;
        output[i+3] = (BYTE)((digest[i/4]>>24)&0xFF) ;
    }
}
...
HDCP_GetVr(BYTE *pVr)
{
    ...
    for(TimeOut = 0 ; TimeOut < 5 ; TimeOut++)
    {
        ...
        pVr[TimeOut*4]   = (ULONG)HDMITX_ReadI2C_Byte(REG_SHA_RD_BYTE1) ;
        pVr[TimeOut*4+1] = (ULONG)HDMITX_ReadI2C_Byte(REG_SHA_RD_BYTE2) ;
        pVr[TimeOut*4+2] = (ULONG)HDMITX_ReadI2C_Byte(REG_SHA_RD_BYTE3) ;
        pVr[TimeOut*4+3] = (ULONG)HDMITX_ReadI2C_Byte(REG_SHA_RD_BYTE4) ;
