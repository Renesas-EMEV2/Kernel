...
/* HDMI output mode notifier to emxx_fb.c */
#include <linux/module.h>
#include <linux/notifier.h>
#include <mach/fbcommon.h>

static RAW_NOTIFIER_HEAD(HDMI_NOTIFIER);
enum EMXX_FB_OUTPUT_MODE HDMI_OUTPUT_MODE = EMXX_FB_OUTPUT_MODE_LCD;

int register_hdmi_notifier(struct notifier_block *nb)
{
	return raw_notifier_chain_register(&HDMI_NOTIFIER, nb);
}
EXPORT_SYMBOL(register_hdmi_notifier);

int unregister_hdmi_notifier(struct notifier_block *nb)
{
	return raw_notifier_chain_unregister(&HDMI_NOTIFIER, nb);
}
EXPORT_SYMBOL(unregister_hdmi_notifier);

static int hdmi_notifier_call_chain(unsigned long val, void *v)
{
	HDMI_OUTPUT_MODE = val;
	return raw_notifier_call_chain(&HDMI_NOTIFIER, val, v);
}
EXPORT_SYMBOL(HDMI_OUTPUT_MODE);
#endif
...
//#define INPUT_AUDIO_SAMPLE_FREQ                     22050
...
#ifdef CONFIG_EMXX_HDMI
			printk("\n===== HDMI removed =====\n");
			hdmi_notifier_call_chain(EMXX_FB_OUTPUT_MODE_LCD, NULL);
#endif
			// unplug mode, ...
			DisableAudioOutput() ;
			DisableVideoOutput() ;
