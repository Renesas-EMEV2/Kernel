...
#endif
static int emxx_ioc_get_overlay_area(struct fb_info *info, unsigned long arg);
#endif
...
static void emxx_fb_shutdown(struct platform_device *pdev);
static int  __devinit emxx_fb_probe(struct platform_device *device);
...
int emxx_fb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case EMXX_GET_OVERLAY_AREA:
		/* get overlay area */
		return emxx_ioc_get_overlay_area(info, arg);
#endif
...
/******************************************************************************
* MODULE   : emxx_ioc_get_overlay_area
* FUNCTION : frame buffer driver ioctl EMXX_GET_OVERLAY_AREA
* RETURN   :  0      : success
*            -EFAULT :
* NOTE     : none
******************************************************************************/
static int emxx_ioc_get_overlay_area(struct fb_info *info, unsigned long arg)
{
	...
	printk_dbg((_DEBUG_FB & 0x01), "ioctl(GET_OVERLAY_AREA) <start>\n");

	/* set Overlay Area start address and size */
	area.addr = OVERLAY_FRAME_BASE;
	area.size = OVERLAY_FRAME_SIZE;

	/* copy to user scrn mode infomation */
	if (copy_to_user((struct emxx_overlay_area *) arg,
		&area, sizeof(struct emxx_overlay_area))) {
		return -EFAULT;
	}

	printk_dbg((_DEBUG_FB & 0x02), "(ioctl(GET_OVERLAY_AREA) <end>\n");
	return 0;
}
#endif
